...
4.  **Ordering Guarantees:**
    - While some brokers (e.g., Kafka with partitions) offer ordering guarantees _within a single stream/partition_, ensuring global ordering across multiple event types or partitions is difficult and often requires careful design or sacrificing parallelism.
...
Ensuring that events are published reliably _and_ that the local database transaction for the change that triggered the event is atomic is a common challenge. If the local transaction commits but the event fails to publish, the system state becomes inconsistent. The Outbox Pattern solves this.
